...
class LLMEngine:
    def __init__(
        ...
        self.num_engines = self.disagg_parallel_config.decoding.data_parallel_size
    ...
    def get_cluster_status(self):
        ...
        return decoding_block_usage, block_usage_index, \
               decoding_scheduler_usage, scheduler_usage_index
    ...
    def get_gpu_status(self, context_block_usage, decoding_block_usage):
        ...
        return decoding_stats, avg_stats

    def log_tb_agg(self, decoding_block_usage, decoding_scheduler_usage, avg_gpu_stats=None):
        metrics = {}
    ...
    async def _start_my_event_loop(self):
        while True:
            decoding_block_usage, block_usage_index, \
            decoding_scheduler_usage, scheduler_usage_index = self.get_cluster_status()
            ...
            print(pd.DataFrame(decoding_block_usage, index=block_usage_index))
            print(pd.DataFrame(decoding_scheduler_usage, index=scheduler_usage_index))

            ### TF metrics computation and logging ###
            self.log_tb_agg(decoding_block_usage, decoding_scheduler_usage, avg_gpu_stats=None)
    ...
    async def generate(
        ...
        self.decoding_cluster.add_request(req)
