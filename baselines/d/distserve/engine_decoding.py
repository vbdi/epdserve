    ...
    async def _step(self) -> None:
        ...
        if len(batched_requests) == 0:
            ...
            for request in batched_requests.requests:
                if request.is_context_stage():
                    self.engine_on_new_lifetime_event_callback(request.request_id, LifetimeEvent(LifetimeEventType.ContextBegin))
                else:
                    self.engine_on_new_lifetime_event_callback(request.request_id, LifetimeEvent(LifetimeEventType.DecodingBegin), True)
                
            # Check if all requests are on GPU (i.e. not swapped out)
            # assert self.block_manager.is_all_requests_on_gpu(batched_requests), "Some requests are currently swapped out to CPU"
            ...
            for req_idx, request in enumerate(batched_requests.requests):
                ...
                seq_md.block_tables = {req_id:block_tables_all[req_idx]}

                if seq_md.is_prompt==False:
            ...
            # print(f'Batch {[(request.request_id, request.is_context_stage()) for request in batched_requests.requests]} ')
        ...
        if len(self.batches_in_pipeline) == self.parallel_config.pipeline_parallel_size:
            ...
            if self.batches_ret_futures[0] is None:
                ...
                finished_batch.finish_one_iteration(generated_tokens, generated_tokens_ids, end_time)

                for request, new_token, new_token_id in zip(finished_batch.requests, generated_tokens, generated_tokens_ids):
                    self.engine_on_new_step_output_callback(request.request_id, StepOutput(request, new_token, new_token_id))
                    if request.is_context_stage()==False and request.vllm_seq_metadata.is_prompt==True: # conext stage just ended for this request
                        request.vllm_seq_metadata.is_prompt = False 
                        self.engine_on_new_lifetime_event_callback(request.request_id, LifetimeEvent(LifetimeEventType.ContextEnd))

                    if request.is_finished:
                        self.engine_on_new_lifetime_event_callback(request.request_id, LifetimeEvent(LifetimeEventType.DecodingEnd))
