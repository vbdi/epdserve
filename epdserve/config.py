...
class ClusterParallelConfig(ParallelConfig):
    def __init__(
        self,
        ...
        super().__init__(tensor_parallel_size,
        ...
        # TODO: LATER WORLD SIZE MIGHT NEED ADJUSTMENT
        # self.world_size = pipeline_parallel_size * tensor_parallel_size
        # self.use_parallel = self.world_size > 1

    def to_list(self) -> List[int]:
        dp_list = [self.data_parallel_size,
                   self.data_parallel_rank]
        tp_pp_list = super().to_list()
        return dp_list + tp_pp_list
...
class EncodingStageSchedConfig:
    """Configuration for the encoding stage scheduler.
...
class ModelConfig:
    ...
    def __init__(
        ...
        if vllm_config is not None:
            self.vllm_engine_config = vllm_config
        else:
            self.vllm_engine_config = self.__get_vllm_config()
        self.run_name = run_name

        try:
            # only the text part as functions below depend on this. Ideally all functions should be replaced by vllm MLLM equivalents.
            self.hf_config = self.vllm_engine_config.model_config.hf_config.text_config
        except:
            self.hf_config = AutoConfig.from_pretrained(self.model, trust_remote_code=self.trust_remote_code)

    def __get_vllm_config(self):
        engine_args = EngineArgs(model=self.model,
                                 max_num_seqs=256,
                                 max_model_len=64,
                                 dtype="float16",
    ...
    # TODO: should be adapted for MLLM
