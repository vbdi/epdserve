...
from vllm import SamplingParams as SamplingParamsVLLM
...
class Request:
    ...
    def __init__(
        ...
        self.vllm_seq_metadata = self.init_vllm_seq_data()
        self.enable_irp = enable_irp
        self.is_sharded = False

    # def get_pixel_data(self, model_config):
    #     from vllm.multimodal.registry import MultiModalRegistry
    #     pixel_data = MultiModalRegistry().map_input(model_config=model_config,
    #                                                 data={'image': self.multi_modal_data})
    #     return pixel_data

    def init_vllm_seq_data(self):
        ds_sp = self.sampling_params
        vllm_sp = SamplingParamsVLLM(temperature=ds_sp.temperature, top_p=ds_sp.top_p, top_k=ds_sp.top_k, max_tokens=ds_sp.max_tokens, stop=ds_sp.stop)
        is_prompt = True
        block_tables=None
        seq_data = SequenceData(array(VLLM_TOKEN_ID_ARRAY_TYPE, self.prompt_token_ids))
        do_sample= True
        pooling_params=None
        token_chunk_size=len(self.prompt_token_ids)
        multi_modal_data={'image': self.multi_modal_data}

        seq_group_metadata = SequenceGroupMetadata(
            request_id=str(self.request_id),
            is_prompt=is_prompt,
            seq_data={self.request_id: seq_data},
    ...
    def get_images_sizes(self):
        return [pil_image.size for pil_image in self.multi_modal_data['image']]
    ...
    def get_actual_output_len(self) -> int: 
        # TODO EPD author: after fix swapping-out bug 
        return self.sampling_params.max_tokens + 1
    ...
    def get_kvcache_slots(self) -> float:
        """The number of kvcache slots needed for the request. The number of kvcache slots is the total number of tokens in the request."""
...
class BatchedRequests:
    ...
    def finish_one_iteration(
        ...
        if generated_tokens_ids is not None and generated_tokens is not None:
...
def create_request(
    ...
    **kwargs
    ...
    return Request(
        ...
        prompt_token_ids=model_input['prompt_token_ids'],
        sampling_params=sampling_params,
        multi_modal_data=model_input['multi_modal_data'],
        enable_irp = kwargs.get('enable_irp', True)
    )
...
class MigratingRequest:
    ...
    Those requests are produced by ContextStageEngine, queued in the "bridge"
    queue, and finally consumed by DecodingStageEngine, which forms a
    ...
    def __init__(
        ...
        self.source_parallel_config = source_parallel_config   
