...
from epdserve.config import ModelConfig, ParallelConfig, CacheConfig
...
class BlockManager:
    ...
    def _get_free_blocks(self, num_blocks: int, location: BlockLocation) -> List[int]:
        ...
        if location == BlockLocation.GPU:
            ...
            assert (num_avail_blocks >= num_blocks), f"not enough free blocks on GPU, requested {num_blocks}, available {num_avail_blocks}"
    ...
    def get_num_blocks_needed(self, request: Request):
        ...
        return (request.get_input_len() + request.get_output_len() + self.cache_config.block_size - 1) // self.cache_config.block_size

    def get_num_append_blocks_needed(self, request: Request) -> int:
        """Get the number of blocks needed for a request already in GPU"""
        assert (self.request_location[request.request_id] == BlockLocation.GPU), f"request {request.request_id} is not on GPU when calling get_num_append_blocks_needed"
    ...
    def allocate_blocks(self, request: Request):
        ...
        if request.request_id not in self.block_table:
            ...
            self.block_table[request.request_id] = self._get_free_blocks(num_blocks_needed, BlockLocation.GPU)
            ...
            if num_blocks_cur < num_blocks_needed:
                self.block_table[request.request_id] += self._get_free_blocks(num_blocks_needed - num_blocks_cur, BlockLocation.GPU)
    ...
    def get_block_usage(self):
        num_cpu_blocks_used = (self.max_num_cpu_blocks - len(self.free_cpu_blocks_list) - len(self.swapping_cpu_blocks_list))
        num_gpu_blocks_used = (self.max_num_gpu_blocks - len(self.free_gpu_blocks_list) - len(self.swapping_gpu_blocks_list))
        safe_div = lambda n,d : n / d if d else 0
        result_dict = {'gpu': f'{round(safe_div(num_gpu_blocks_used, self.max_num_gpu_blocks)*100)}% ({num_gpu_blocks_used}/ {self.max_num_gpu_blocks})',
                       ...
                       '#req': f'{self.block_table.__len__()}'
        }
        return result_dict
    ...
    and then call Worker.swap_blocks on every worker.

    scheduler -> LLMEngine.swap_in_requests ->
        BlockManager.swap_in_requests -> Worker.swap_blocks
    ...
    def swap_requests(self, requests: List[Request], is_swap_in: bool):
        ...
        for request in requests:
            assert (request.request_id in self.block_table), f"request {request.request_id} not allocated"
            assert (self.request_location[request.request_id] == cur_location), f"request {request.request_id} is on {target_location} now"
        ...
        self.engine_remote_call_all_workers_async("swap_blocks", requests, source_block_ids, target_block_ids, is_swap_in)
...
class VisionBlockManager(BlockManager):
    def get_num_blocks_needed(self, request: Request):
        if "llava" in self.model_config.model:
            return sum([1 for img_size in request.get_images_sizes()])
        elif "MiniCPM-V-2_6" in self.model_config.model:  # minicpmv
            return sum([10 if img_size==(4032, 3024) else 1 for img_size in request.get_images_sizes()])
        elif "InternVL2" in self.model_config.model:
            return sum([13 for img_size in request.get_images_sizes()])
        else:
